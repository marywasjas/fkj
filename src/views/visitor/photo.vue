  ...
  <div class="app-container">
    ...
    <div class="fkj-footer">
      <div class="fkj-footer-left">
        <v-touch tag="span" class="my-button" @tap="router('/visitor/phoneNumber')">上一步</v-touch>
      </div>
      ...
      <div class="fkj-footer-right">
        <v-touch  tag="span" class="my-button" @tap="commitPhoneInfo">下一步</v-touch>
      </div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      commitPhoneInfo() {
        if (this.data_uri) {
          this.$store.commit('visitorFrom', { type: 'visitorPhoto', value: this.data_uri })
          this.router('/visitor/reason')
        } else {
          this.error('请先拍取照片！')
        }
      },
      reset() {
        this.data_uri = null
        this.$store.commit('visitorFrom', { type: 'visitorPhoto' })
      },
      base64urlSize(url) {
