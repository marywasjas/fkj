<template>
  <div class="app-container">
    <div class="fkj-content">
      <div class="title">
        <span v-if="backObj.length<=0">请选择要添加的待认证车位</span>
        <v-touch v-if="backObj.length>0" tag="span">已选：{{backObj[0].name}}楼</v-touch>
      </div>
        ...
        <div ref="box" class="box">
          <div :class="show?'show':''" v-for="(item, index) in now" :key="index">
            <v-touch tag="div"  :class="'my-button' + (it.id ? ' room' : '')" v-for="(it, i) in item"
                     @tap="main(it)" :key="i">{{it.name}}{{backObj.length===0?'楼':'号'}}</v-touch>
          </div>
        </div>
      </v-touch>
      <div>
  ...
  export default {
    ...
    methods: {
      ...
      commit(id, name) {
        this.$openloading()
        const _this = this
        const list = _this.$store.state.ownerAuth.parking || []
        const A = []
        list.forEach(function(item) {
          A[item.id] = true
        })
        if (A[id]) {
          _this.error('列表中已存在该车位，不能重复选择哦')
          _this.$closeLoading()
          return
        }
        list.push({ id: id, name: this.backObj[0].name + '楼' + name + '号' })
