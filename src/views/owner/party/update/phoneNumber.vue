...
<script type="text/babel">
  ...
  export default {
    ...
    mounted() {
      console.log(this.partyObj, 'partyObj')
      this.reInput()
    },
    beforeDestroy() {
      this.isDes = true
    ...
    methods: {
      ...
      focusclick() {
        ...
        testValidNum({ phone: phoneNum, code: validNum }).then(response => {
          if (response.code === 0) {
            ownerPartyUpdate({ phone: phoneNum, id: this.partyObj.id, ownerId: this.ownerId }).then(response => {
              if (response.code === 0) {
                const partyList = _this.$store.state.ownerInfo.partyList || []
                const partyObj = _this.$store.state.now.party || null
                partyObj.phone = phoneNum
