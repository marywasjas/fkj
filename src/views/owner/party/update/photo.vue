  ...
  <div class="app-container">
    ...
    <div class="fkj-footer">
      <div class="fkj-footer-left">
        <v-touch  tag="span" class="my-button" @tap="router('/owner/partyDetails')"><i class="el-icon-arrow-left"></i>取消</v-touch>
      </div>
      ...
      <div class="fkj-footer-right">
        <v-touch  tag="span" class="my-button" @tap="commitPhoneInfo">确认修改</v-touch>
      </div>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      commitPhoneInfo() {
        if (this.data_uri) {
          const _this = this
          _this.$openloading()
          ownerPartyUpdate({ image: _this.data_uri, id: this.partyObj.id, ownerId: this.ownerId }).then(response => {
            _this.$closeLoading()
            ...
            if (response.code === 0) {
              ...
              partyObj.picture = _this.data_uri
              ...
              _this.error('提交失败！' + response.msg, function() {
              })
            }
          }).catch(error => {
            _this.$closeLoading()
