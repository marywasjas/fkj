        ...
        <div ref="box" class="houseParkingBox">
          <div :class="show?'show':''" v-for="(item, index) in now" :key="index">
            <div class="my-button box-btn readonly" v-for="(it, i) in item" :key="i">
              <div class="btn-main">
                <div class="btn-main-title"><i style="color: #ffa225;" class="iconfont icon-dian"></i>{{it.name}}</div>
                <div v-if="it.status===1||it.status===0" class="tishi">
                <i class="iconfont icon-dengdai"></i>
                <div>认证中</div>
              </div>
                <div v-if="it.status===2" class="tishi btn">
                  <i class="iconfont icon-shibai"></i>
                  <div>认证失败</div>
                </div>
                <v-touch  v-if="it.status===2" tag="div" @tap="deleteFn(it)" class="my-button btn-main-btn yellow-btn">删 除</v-touch>
                <v-touch v-if="it.status===3" tag="div" class="my-button btn-main-btn blue-btn"  @tap="main(it)">房屋关系人</v-touch>
                <v-touch  v-if="it.status===3" tag="div" @tap="pay(it)" class="my-button btn-main-btn yellow-btn">缴 费</v-touch>
              </div>
            </div>
          </div>
          <div class="tips" v-if="now.length===0">
            您还没有认证房屋，请点击下方‘添加’按钮进行添加
          </div>
        </div>
      </v-touch>
      <div>
...
<script>
  import { ownerHouseRemove } from '@/api/owner'
  import { getHouse } from '@/api/owner'
  export default {
    name: 'phone',
    data() {
      return {
    ...
    mounted() {
      ...
      this.$store.commit('now', { type: 'house' })
      this.reset()
      const _this = this
      getHouse({ userId: _this.ownerId }).then(response => {
        if (response.code === 0) {
          _this.$store.commit('ownerInfo', { type: 'houseList', value: response.data || [] })
    ...
    methods: {
      reset() {
        this.now = this.Handle(this.houseList, 3)
      },
      switchTab(e) {
        if (e === 'right') {
      ...
      addFn() {
        this.router('/owner/houseAdd')
      },
      deleteFn(en) {
        this.$openloading()
        const _this = this
        const e = _this.$deepClone(en)
        _this.confirm('确定将认证失败的(' + e.name + ')房屋从列表移除吗？', function() {
          _this.$openloading()
          ownerHouseRemove({ ownerId: _this.ownerId, houseId: e.id }).then(response => {
            if (response.code === 0) {
              _this.houseList.forEach(function(item, index, arr) {
                if (e.id === item.id) {
                  arr.splice(index, 1)
                }
              })
              _this.$store.commit('ownerInfo', { type: 'houseList', value: _this.houseList })
              _this.reset()
            } else {
              _this.error('删除失败！')
            }
      ...
      main(e) {
        this.router('/owner/houseParty')
        this.$store.commit('now', { type: 'house', value: e })
      },
      pay(e) {
        this.router('/owner/housePayType')
        this.$store.commit('now', { type: 'house', value: e })
      }
    }
  }
