  ...
  <div class="app-container">
        ...
        <div class="payInfo">
          <el-form label-width="15vw" label-suffix=":">
            ...
            <div v-if="(payInfo.money + payInfo.punishMoney)>0" class="pay-type-box">
              ...
              <v-touch class="my-button readonly" tag="li"> <!-- @tap="pay('微信')" -->
    ...
    <div class="fkj-footer">
      ...
      <div v-else class="fkj-footer-left">
        <v-touch tag="span" class="my-button" @tap="router('/pay/water/phoneNumber')">上一步</v-touch>
      </div>
    </div>
  ...
  export default {
    data() {
      return {
        ...
        url: '',
        data: null,
        payInfo: this.$store.state.payInfo.water,
        money: 0
      }
    },
    mounted() {
      ...
      setTO(orderId) {
        ...
        getPayStatus({ orderNo: orderId }).then(response => {
          if (response.code === 0) {
            ...
            setTimeout(() => {
              _this.router('/pay/water/success')
            }, 2000)
          } else {
