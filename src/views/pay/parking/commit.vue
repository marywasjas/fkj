  ...
  <div class="app-container">
    ...
    <div class="fkj-footer">
      <div v-if="payInfo.ownerId" class="fkj-footer-left">
        <v-touch tag="span" class="my-button" @tap="router('/owner/parkingPayType')">取消</v-touch>
      </div>
      <div v-else class="fkj-footer-left">
        <v-touch tag="span" class="my-button" @tap="router('/pay/parking/phoneNumber')">上一步</v-touch>
      </div>
    </div>
    ...
    <el-dialog title="车位费欠缴明细" :visible.sync="dialog.dialogVisible_list"
               top="21vh" width="40%">
  ...
  export default {
    data() {
      return {
        ...
        timer: false,
        data: null,
        payInfo: this.$store.state.payInfo.parking,
        money: 0
      }
    },
    mounted() {
      ...
      setTO(orderId) {
        ...
        getPayStatus({ orderNo: orderId }).then(response=> {
          if (response.code === 0) {
            ...
            setTimeout(() => {
              _this.router('/pay/parking/success')
            }, 2000)
          } else {
