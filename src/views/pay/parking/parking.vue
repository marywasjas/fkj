<template>
  <div class="app-container">
    <div class="fkj-content">
      <div class="title">
        <span v-if="backObj.length<=0">请选择要缴车位费的车位</span>
  ...
  export default {
    ...
    methods: {
      ...
      commit(en) {
        ...
        getParkingPayInfo({ parkId: e.id }).then(response => {
          if (response.code === 0) {
            _this.router('/pay/parking/phoneNumber')
            _this.$store.commit('payInfo', { type: 'parking', value: {
              parkingId: e.id,
              info: response.data
            }})
          } else {
