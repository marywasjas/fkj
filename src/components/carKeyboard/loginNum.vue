<template>
  <v-touch tag="div" class="num-box">
    ...
    <div style="padding-left: 2vw;">
      <v-touch tag="li" :class="'my-button' + (isReadonly ? ' readonly':'')" v-for="(item, i) in codeList.substring(0,10)"
               :key="i+20" @tap="number(item)">{{item}}</v-touch>
    </div>
    <div>
      <v-touch tag="li" :class="'my-button' + (isReadonly ? ' readonly':'')"  v-for="(item, i) in codeList.substring(10,19)"
               :key="i+40" @tap="number(item)">{{item}}</v-touch>
    </div>
    <div style="padding-left: 1vw;">
      <v-touch tag="li" :class="'my-button' + (isReadonly ? ' readonly':'')"  v-for="(item, i) in codeList.substring(19,26)"
               :key="i+60" @tap="number(item)">{{item}}</v-touch><v-touch
      style="width: 7vw;" tag="li" class="my-button del-btn" @tap="deleteFn">删除</v-touch>
    </div>
  </v-touch>
</template>
  ...
  export default {
    data() {
      return {
        ...
        isReadonly: false,
        color: '',
        codeList: 'QWERTYUIOPASDFGHJKLZXCVBNM'
      }
    },
    props: {
      limit: {
